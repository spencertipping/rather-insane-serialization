...
var rather_insane_serialization = {
  serialize: function (x) {
    ...
    // 'refs' table is updated with a new entry and a variable is emitted as the
    // serialized output.
    var visit = function (base, index, x) {
      ...
      if (x[secret]) {
        ...
        // and return its ref as the value.
        initializers.push([base, index, '_' + x[secret].toString(36)]);
        return '_' + x[secret].toString(36);
      }

      // Mark the object as having been visited. We then visit each of the
      // children; doing so side-effectfully creates initialization statements.
      ...
      switch (x.constructor) {
        case Array:
          for (var pieces = [], i = 0, l = x.length; i < l; ++i)
            initializers.push([x_id, i, visit(x_id, i, x[i])]);
        ...
        case Function:
          for (var k in x)
            if (x.hasOwnProperty(k) && k !== secret)
              initializers.push([x_id, k, visit(x_id, k, x[k])]);

          visit(x_id, 'prototype', x.prototype);
          refs[x_id] = x.toString();
          break;
      ...
      if (base) initializers.push([base, index, '_' + x_id.toString(36)]);
      return '_' + x_id.toString(36);
    };
